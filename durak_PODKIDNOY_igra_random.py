...
def player_turn():
        ...
        if len(player) > 0 and len(computer) > 0:
            if c =='taken':
                for _ in range(min(min(len(computer), 6 - cnt), len(player))):
                    print('Подкидывайте по одной или напишите "бито":')
                    while True:
                        p = input()
                        if p.lower() == 'бито':
                            break
                ...
                print('Вы можете подкинуть:',', '.join(turn_list),'// или напишите "бито".')
                while True:
                    p = input()
                    if p.lower() == 'бито':
                        break
    ...
    if len(deck) > 0: dobor_kart(player)
...
def computer_turn():
    ...
    if len(deck) > 0: dobor_kart(computer)
    return player, computer, deck, winner
